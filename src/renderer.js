...
const replaySoundSw     = document.getElementById('replay-sound-switch');
const replaySoundField  = document.getElementById('replay-sound-field');
const replaySoundSel    = document.getElementById('replay-sound-select');
const replaySoundTest   = document.getElementById('replay-sound-test');
const replaySoundTip    = document.getElementById('replay-sound-tip');
...
// The sounds are generated by assets/source/mksounds.py, all to the same loudness, so choosing one
// changes its character and never its volume.
const SAVE_SOUNDS = {
  chime:   'Two bell notes, rising a fifth. Warm, and over in about a second.',
  pop:     'One soft note that slides up into place. The shortest of the three, and the one most likely to be lost under a busy game.',
  sparkle: 'A quick four-note run in glass. The most celebratory, and the easiest to pick out over loud audio.',
};
...
function applyReplayUi() {
  ...
  const soundOn = c.soundOnSave !== false;
  replaySoundSw.classList.toggle('on', soundOn);
  replaySoundSel.value = SAVE_SOUNDS[c.saveSound] ? c.saveSound : 'chime';
  replaySoundTip.textContent = SAVE_SOUNDS[replaySoundSel.value];
  // Only after the replay switch has had its say: this is off with replay off *or* sound off.
  if (!soundOn) replaySoundField.classList.add('off');
  renderForeground();
...
// A fresh element per play, so a second save straight after the first gets its own sound rather
// than restarting the one still ringing. Nothing is held open between saves.
function playSaveSound(name) {
  const file = SAVE_SOUNDS[name] ? name : 'chime';
  new Audio(`../assets/sounds/${file}.wav`).play().catch(() => {});
}

replaySoundSw.addEventListener('click', () => patchReplay({ soundOnSave: replayConfig.soundOnSave === false }));
replaySoundSel.addEventListener('change', () => {
  // Choosing one plays it: a name alone says nothing about what it sounds like.
  playSaveSound(replaySoundSel.value);
  patchReplay({ saveSound: replaySoundSel.value });
});
replaySoundTest.addEventListener('click', () => playSaveSound(replaySoundSel.value));
...
api.onCaptureEvent((event) => {
  switch (event.event) {
    ...
    case 'config':
      ...
      break;
    case 'clip-saved':
      if (replayConfig && replayConfig.soundOnSave !== false) playSaveSound(replayConfig.saveSound);
